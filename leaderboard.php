...
<div class="leaderboard-container">
    <h2 class="section-header">Quiz Leaderboard - All Attempts</h2>
    <table class="leaderboard-table">
        <thead>
        <tr>
            <th>Rank</th>
        ...
        <tbody>
        <?php
        $sql_leaderboard_all = "SELECT User.Username, Quiz_Points.Points
                                FROM Quiz_Points 
                                INNER JOIN User ON Quiz_Points.User_ID = User.User_ID
                                ORDER BY Quiz_Points.Points DESC";
        $result_leaderboard_all = mysqli_query($conn, $sql_leaderboard_all);
        $rank = 1;
        while ($row_leaderboard_all = mysqli_fetch_assoc($result_leaderboard_all)) {
            echo "<tr>";
            echo "<td>{$rank}</td>";
            echo "<td>{$row_leaderboard_all['Username']}</td>";
            echo "<td>{$row_leaderboard_all['Points']}</td>";
            echo "</tr>";
            $rank++;
        }
        ?>
